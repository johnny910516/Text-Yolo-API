...
class TextYolo():    
    def write_txt(self, text, output_path):

        with open(os.path.join(output_path, 'text.txt'), 'w', encoding='utf-8') as file:
            file.write(text)
    ...
    def saveResult(self, args, image_index, image_amount, txt_content, htr_model, image_path, image, sort_textbox_coordinate, text_coordinate, caret_dict, output_path, text_textbox_split_path, split_path, post_process_split_path):
        ...
        new_sort_text_coordinate = self.insert_paragraph_mark(args, image_index, sort_text_coordinate, sort_textbox_coordinate)
        ...
        if args.test_mode:
            if average_resolution > args.resoultion_threshold:
                ...
                if image_index == image_amount-1:
                    self.write_txt(txt_content[0], args.output)
            else:
